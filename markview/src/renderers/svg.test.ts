...
describe('safeSvgElement', () => {
  ...
  it('strips javascript: in namespaced xlink:href', () => {
    const svg = safeSvgElement('<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><a xlink:href="javascript:bad()"><text>x</text></a></svg>');
  ...
  it('rejects input whose root is not an SVG element', () => {
    expect(() => safeSvgElement('<div>not svg</div>')).toThrow('Renderer returned invalid SVG');
    expect(() => safeSvgElement('')).toThrow('Renderer returned invalid SVG');
  });

  it('preserves camelCase SVG attributes that renderers depend on', () => {
    const svg = safeSvgElement('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 20" preserveAspectRatio="xMidYMid meet"></svg>');
    expect(svg.getAttribute('viewBox')).toBe('0 0 10 20');
    expect(svg.getAttribute('preserveAspectRatio')).toBe('xMidYMid meet');
  });

  // Regression: Mermaid serialises htmlLabels inside <foreignObject> as HTML, so
  // a line break emits a void <br>. An XML parse rejects that outright, which
  // used to replace the whole diagram with a render error.
...
describe('safeSvgElement against real Mermaid output', () => {
  beforeAll(() => {
